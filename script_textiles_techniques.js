...
function displayColorGrid(images, isFilteringMaterial = false) {
  ...
  images.forEach((imageData) => {
    ...
    img.src = isFilteringMaterial
      ? imageData.image.thumbnail + "&max=100"
      : imageData.image + "&max=100";
    img.style.width = "100%";
...
fetch("./data/data_NEW.json")
  ...
  .then((jsonData) => {
    const imagePaths = jsonData.slice(0, 2000).map((item) => ({
      ...
      image: item.image.thumbnail + "&max=100", //thumbnail URL
      id: item.id,
...
fetch("./data/data_NEW.json")
  ...
  .then((data) => {
    ...
    // Normalize materials for case-insensitive comparison
